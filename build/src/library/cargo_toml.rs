...
fn base_cargo_toml(package_short_name: &str, package_name: &str) -> String {
    indoc::formatdoc!(
        r#"
    # ------------------------------------------------------------------------------------------
    ...
    [package]
    name = "leptos-icons-{package_short_name}"
    version = "0.0.1"
    authors = ["Charles Edward Gagnon"]
    edition = "2021"
    description = "{package_name} library for the leptos web framework"
    readme = "./README.md"
    repository = "https://github.com/Carlosted/leptos-icons/leptos-icons-{package_short_name}"
    license = "MIT"
...
impl CargoToml {
    ...
    #[instrument(level = "info")]
    pub(crate) async fn init(&mut self, package_meta: &PackageMetadata) -> Result<()> {
        info!("Writing BASE_CARGO_TOML content.");
        self.create_file()
            .await?
            .write_all(
                base_cargo_toml(&package_meta.short_name, &package_meta.package_name).as_bytes(),
            )
            .await
            .map_err(Into::into)
    }
